...
public class MainPolicy {
    ...
    public static void filtering_premiumAMount(List<Policy> policyList) {
        policyList.stream().filter(p -> p.getPremiumAmount() > 1200).forEach(System.out::println);
    }
    ...
    public static void total_premium(List<Policy> policyList) {
        double totalPremium = policyList.stream().mapToDouble(Policy::getPremiumAmount).sum();
    ...
    public static void premium_Range(List<Policy> policyList) {
        List<Policy> filteredList = policyList.stream().filter(p -> p.getPremiumAmount() > 1000 && p.getPremiumAmount() < 2000) // correct condition
    ...
    public static void highest_premium(List<Policy> policyList) {
        int maximumAmount = policyList.stream().mapToInt(p -> (int) p.getPremiumAmount()).max().orElse(-1);
    ...
    public static void groupby(List<Policy> policyList) {
        Map<Character, List<Policy>> groupBy = policyList.stream().collect(Collectors.groupingBy(p -> p.getHolder_Name().charAt(0)));
    ...
    public static void averagePremium(List<Policy> policyList) {
        double avg = policyList.stream().mapToDouble(Policy::getPremiumAmount) // better to use double
                .average().orElse(0.0); // default if list is empty
    ...
    public static void sort_premiumAmount(List<Policy> policyList) {
        policyList.sort((p1, p2) -> (p1.getPremiumAmount() < p2.getPremiumAmount()) ? -1 : (p1.getPremiumAmount() > p2.getPremiumAmount()) ? 1 : 0);
    ...
    public static void filtering_premiumAMount_Range(List<Policy> policyList) {
        policyList.stream().filter(p -> p.getPremiumAmount() > 2000).forEach(System.out::println);
    }
    ...
    public static void unique_holder_name(List<Policy> policyList) {
        policyList.stream().map(Policy::getHolder_Name).distinct().collect(Collectors.toList());
        System.out.println("Unique holderName:");
        policyList.forEach(System.out::println);
    }

    //By using the HolderName find Policy
    public static void find_holder_name(List<Policy> policyList) {
        List<Policy> policyWithHolderName = policyList.stream().filter(p -> p.getHolder_Name().toLowerCase().contains("prasanna")).collect(Collectors.toList());
        System.out.println(" Prasanna:");
        policyWithHolderName.forEach(System.out::println);
    }


    // 14. Create a Map of Policy Numbers to Premium Amounts
    public static void policy_map(List<Policy> policyList) {
        Map<String, Double> map_policy = policyList.stream()
                .collect(Collectors.toMap(
                        Policy::getHolder_Name,
                        Policy::getPremiumAmount
                ));
        System.out.println("Policy Number -> Premium Map: " + map_policy);
    }
    ...
    public static void main(String[] args) {
        ...
        //Unique Holder Name
        unique_holder_name(policyList);
        //Using holder Name find present in list or not
        find_holder_name(policyList);
        //14. Create a Map of Policy Numbers to Premium Amounts
        policy_map(policyList);
    }
}